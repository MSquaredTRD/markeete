...
export default function Home() {
  ...
  const [maxLiability, setMaxLiability] = useState(0n);
  ...
  const [claimForAddress, setClaimForAddress] = useState('');
  ...
  const refresh = useCallback(async (activeAccount?: Address) => {
    setLoading(true);
    try {
      const [nextProductId, nextOrderId, listing, purchase, action, accrued, liability, capacity, isPaused, invariant] = await Promise.all([
        ...
        publicClient.readContract({ address: ESCROW_ADDRESS, abi: escrowAbi, functionName: 'maxTotalLiability' }),
      ...
      setMaxLiability(capacity); setPaused(isPaused); setSolvent(invariant);
      if (activeAccount) {
  ...
  return (
    <main>
      ...
      <footer><span>markeete.online</span><span>Experimental · unaudited · use at your own risk</span><span className="footer-links"><button onClick={() => setTab('Docs')}>Docs</button><a href="/risk-disclosure.md" target="_blank">Risks</a><a href={`${BASESCAN_URL}/address/${ESCROW_ADDRESS}`} target="_blank" rel="noreferrer">Contract</a></span></footer>
    </main>
  );
}
